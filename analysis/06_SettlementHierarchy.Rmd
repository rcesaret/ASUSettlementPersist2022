...
# Package names
packages <- c("rgdal", "rgeos", "sp", "sf", "GISTools", "raster", "Matrix", "gdistance", "lwgeom", "tidyverse", "tidyr", "classInt", "pacman", "RColorBrewer", "cowplot")#, "data.table", "zoo", "era", "JOPS", "mgcv","igraph", "ggnewscale", "ggrepel","ggridges", "movecost",  "datplot", "scales",
...
CatchLims.sf = st_as_sf(CatchLims)
Catch.sf = st_as_sf(Catch_List[[p]])
Hillshade.s <- st_as_stars(HillshadeLake)
...
ggp1 = ggplot() +  geom_stars(data = Hillshade.s)+
  ...
  geom_point(data=x@data, mapping=aes(x=East, y=North, fill=as.factor(km_breaks), size=Population.s2), shape=21, color = "black", stroke = 1.05, alpha=0.55)+
...
ggp2 = ggplot() +  geom_stars(data = Hillshade.s)+
  ...
  theme(legend.justification=c(0,1), legend.position=c(0.03,0.9), 
        ...
        legend.background = element_rect(colour = 'black', fill = 'white', linetype='solid'),
        plot.subtitle = element_text(hjust = 0.5, face="bold", size=16))
  #scale_fill_discrete(pal1)legend.title=element_rect(face="bold"),


plot_row=plot_grid(ggp2, ggp1, align = "h", nrow = 1, rel_heights = c(1/2, 1/2))
title <- ggdraw() + 
  draw_label("SBOM Kmeans Settlement Hierarchy Classes",fontface = 'bold',x = 0.5,hjust = 0.5, size=20 )
...
dat$SetHierLevel=xx$SetHierLevel
```
