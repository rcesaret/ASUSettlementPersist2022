...
--global average for a metric; value as a a fraction of the global value
--OR as a Z-score...


## Concentration/Dispersion Metrics

Location Quotient and similar
...
```{r, 'Global Concentration/Dispersion Metrics', message=FALSE,warning=FALSE}

for (i in 1:length(Poly_List)){
  #thresh = 1
  #w <- TrsprtNet_CDmatList[[i]]
  #w[w > thresh] <- 0
  #w[w > 0] <- 1
  #diag(w) <- 1
  ...
  Poly_List[[i]]$FLQ_UrbPop <- FLQ.dist(df=df,e="UrbanPop.s2", E="Population.s2",m=m, r=1)
  Poly_List[[i]]$spGini_Pop <- GiniSp.dist(df=df, var="Population.s2", m=m, r=1)
  Poly_List[[i]]$spGini_UrbPop <- GiniSp.dist(df=df, "UrbanPop.s2", m=m, r=1)
  
  Poly_List[[i]]$LQ_UrbOccuTime <- LQ(df=df, e="UrbOccuTime", E="OccuTime")
  Poly_List[[i]]$FLQ_UrbOccuTime <- FLQ.dist(df=df, e="UrbOccuTime", E="OccuTime",m=m, r=1)
  Poly_List[[i]]$spGini_OccuTime <- GiniSp.dist(df=df, "OccuTime", m=m, r=1)
  Poly_List[[i]]$spGini_UrbOccuTime <- GiniSp.dist(df=df, "UrbOccuTime", m=m, r=1)
  
  Poly_List[[i]]$LQ_UrbOccuInertia <- LQ(df=df, e="UrbOccuInertia", E="OccuInertia")
  Poly_List[[i]]$FLQ_UrbOccuInertia <- FLQ.dist(df=df, e="UrbOccuInertia", E="OccuInertia",m=m, r=1)
  Poly_List[[i]]$spGini_OccuInertia <- GiniSp.dist(df=df, "OccuInertia", m=m, r=1)
  Poly_List[[i]]$spGini_UrbOccuInertia <- GiniSp.dist(df=df, "UrbOccuInertia", m=m, r=1)
  
  Poly_List[[i]]$LQ_Pct_UrbdeltaPop12 <- LQ(df=df,e="Pct_UrbdeltaPop12", E="Pct_deltaPop12")
  Poly_List[[i]]$FLQ_Pct_UrbdeltaPop12 <- FLQ.dist(df=df, e="Pct_UrbdeltaPop12", E="Pct_deltaPop12",m=m, r=1)
  Poly_List[[i]]$spGini_Pct_deltaPop12 <- GiniSp.dist(df=df, "Pct_deltaPop12", m=m, r=1)
  Poly_List[[i]]$spGini_Pct_UrbdeltaPop12 <- GiniSp.dist(df=df, "Pct_UrbdeltaPop12", m=m, r=1)
  
  Poly_List[[i]]$LQ_PopDens <- LQ(df=df, e="Population.s2", E="Area_ha")
  Poly_List[[i]]$FLQ_PopDens <- FLQ.dist(df=df, e="Population.s2", E="Area_ha",m=m, r=1)
  Poly_List[[i]]$spGini_Area_ha <- GiniSp.dist(df=df, "Area_ha", m=m, r=1)
  
  Poly_List[[i]]$LQ_Catch_Popdens <- LQ(df=df, e="Population.s2", E="Catchment_ha")
  Poly_List[[i]]$FLQ_Catch_Popdens <- FLQ.dist(df=df, e="Population.s2", E="Catchment_ha",m=m, r=1)
  Poly_List[[i]]$spGini_Catchment_ha <- GiniSp.dist(df=df, "Catchment_ha", m=m, r=1)
  
  Poly_List[[i]]$LQ_Urb_Abs_rPert <- LQ(df=df, e="Urb_Abs_rPert", E="Abs_rPert")
  Poly_List[[i]]$FLQ_Urb_Abs_rPert <- FLQ.dist(df=df, e="Urb_Abs_rPert", E="Abs_rPert", m=m, r=1)
  Poly_List[[i]]$spGini_Abs_rPert <- GiniSp.dist(df=df, "Abs_rPert", m=m, r=1)
  Poly_List[[i]]$spGini_Urb_Abs_rPert <- GiniSp.dist(df=df, "Urb_Abs_rPert", m=m, r=1)
  
  Poly_List[[i]]$LQ_Q.Urb_rPert <- LQ(df=df, e="Q.Urb_rPert", E="Q.rPert")
  Poly_List[[i]]$FLQ_Q.Urb_rPert <- FLQ.dist(df=df, e="Q.Urb_rPert", E="Q.rPert",m=m, r=1)
  Poly_List[[i]]$spGini_Q.rPert <- GiniSp.dist(df=df, "Q.rPert", m=m, r=1)
  Poly_List[[i]]$spGini_Q.Urb_rPert <- GiniSp.dist(df=df, "Q.Urb_rPert", m=m, r=1)
  ...
  Poly_List[[i]]$spGini_FwOvlp.Pop <- GiniSp.dist(df=df, "FwOvlp.Pop", m=m, r=1)
  Poly_List[[i]]$spGini_BwOvlp.Pop <- GiniSp.dist(df=df, "BwOvlp.Pop", m=m, r=1)
  Poly_List[[i]]$spGini_FwOvlp.Area <- GiniSp.dist(df=df, "FwOvlp.Area", m=m, r=1)
  Poly_List[[i]]$spGini_BwOvlp.Area <- GiniSp.dist(df=df, "BwOvlp.Area", m=m, r=1)
  
  Poly_List[[i]]$spGini_PopPressure <- GiniSp.dist(df=df, "PopPressure", m=m, r=1)
  Poly_List[[i]]$spGini_centr_avg <- GiniSp.dist(df=df, "centr_avg", m=m, r=1)
  Poly_List[[i]]$spGini_centr_deg <- GiniSp.dist(df=df, "centr_deg", m=m, r=1)
  Poly_List[[i]]$spGini_centr_btw <- GiniSp.dist(df=df, "centr_btw", m=m, r=1)
  Poly_List[[i]]$spGini_centr_eig <- GiniSp.dist(df=df, "centr_eig", m=m, r=1)
  Poly_List[[i]]$spGini_centr_clos <- GiniSp.dist(df=df, "centr_clos", m=m, r=1)
  Poly_List[[i]]$spGini_centr_hrmo <- GiniSp.dist(df=df, "centr_hrmo", m=m, r=1)
  Poly_List[[i]]$spGini_centr_hub <- GiniSp.dist(df=df, "centr_hub", m=m, r=1)
  Poly_List[[i]]$spGini_centr_auth <- GiniSp.dist(df=df, "centr_auth", m=m, r=1)
  Poly_List[[i]]$spGini_centr_pgrk <- GiniSp.dist(df=df, "centr_pgrk", m=m, r=1)
  Poly_List[[i]]$spGini_trans_loc <- GiniSp.dist(df=df, "trans_loc", m=m, r=1)
  Poly_List[[i]]$spGini_SetHierLevel <- GiniSp.dist(df=df, "SetHierLevel", m=m, r=1)
}
```
...
ordering <- c(
  ...
  #STEP #6 SETTLEMENT HIERARCHY/DEMOG SPATIAL GINIS
       ...
       "spGini_PopPressure","spGini_centr_avg","spGini_centr_deg","spGini_centr_btw",
