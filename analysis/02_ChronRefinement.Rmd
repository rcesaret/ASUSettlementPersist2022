...
MP = data.frame(years = c(CHRON$Begin, CHRON$End), era = c(CHRON$Begin.era,CHRON$End.era))
  ...
  saveRDS(MP2, file = paste0(wd$data_p, "ModellingPeriods_df.RData"))
  saveRDS(MyBreaks, file = paste0(wd$data_p, "ModellingPeriod_AxisBreaks.RData"))
  saveRDS(MyLabs, file = paste0(wd$data_p, "ModellingPeriod_AxisLabels.RData"))
...
```{r, 'Beta Dist Popularity Plot', message=FALSE, warning=FALSE, echo=FALSE }
ggp1 <- ggplot() + geom_density(BDistFreq, mapping=aes(Time, ..scaled..*0.00979), fill = "black", alpha=0.35) + 
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, color="black", size=9), 
          ...
          legend.background = element_rect(colour = 'black', fill = 'white', linetype='solid'), plot.title = element_text(hjust = 0.5, face="bold"))+theme(plot.background = element_rect(fill = "white", colour = NA))

#save figure
ggsave("BetaDistPopularity.png", plot = ggp1, device = "png", path = wd$figs, scale = 1, width = 8, height = 5.143,   units = "in",  dpi = 1500)

rm(ggp1)

# import figure for display
knitr::include_graphics(paste0(wd$figs,"BetaDistPopularity.png"), FALSE)
```
...
#save figure
ggsave("AoristicVar2Popularity.png", plot = ggp1, device = "png", path = wd$figs, scale = 1, width = 8, height = 5.143,   units = "in",  dpi = 1500)

rm(ggp1)

# import figure for display
knitr::include_graphics(paste0(wd$figs,"AoristicVar2Popularity.png"), FALSE)
```
...
knitr::kable(PC.mat, "pipe" ,align="c")
...
#save figure
ggsave("BayesCompareVar2PC.png", plot = ggp1, device = "png", path = wd$figs, scale = 1, width = 7, height = 4.5,   units = "in",  dpi = 1500)

rm(ggp1)

# import figure for display
knitr::include_graphics(paste0(wd$figs,"BayesCompareVar2PC.png"), FALSE)
...
```{r, 'spda.ggplot trajectory plot outputs', echo=FALSE, message=FALSE, warning=FALSE}

############################ UPDF_example.png ###################################

ex1.site <- Agg[which(Agg$SubOccSeqLoc=='TX-IX-1-A'),]
ex1.site_NoOvlp <- Agg_NoOvlp[which(Agg_NoOvlp$SubOccSeqLoc=='TX-IX-1-A'),]
...
plt = spda.ggplot(ex1, method="bayesian", 
                  pc.method="uniform", 
                  legend_position = "top", 
                  PopLogScale = F, 
                  xlabels = MyLabs, 
...
plt = plt + theme(plot.caption = element_blank())+theme(plot.background = element_rect(fill = "white", colour = NA))

#save figure
ggsave("UPDF_example.png", plot = plt, device = "png", path = wd$figs, scale = 1, width = 7, height = 4.5,   units = "in",  dpi = 1500)


############################ SettlementPopTraj.png ###################################
...
ggp <- plot_grid(plt, plt2, align = "v", nrow = 2, rel_heights = c(1/2, 1/2))+theme(plot.background = element_rect(fill = "white", colour = NA))
...
plt = plt + labs(title = "'Chronologically Refined' Settlement Population Time Series", subtitle="S. Basin of Mexico; Site CH-186-A (500 BC - AD 1520)", y = "Occupational Probability") + theme(plot.caption = element_blank(), axis.title.y = element_text(color="black", size=12, face="bold"))+theme(plot.background = element_rect(fill = "white", colour = NA))

#save figure
ggsave("SettlementPopTraj.png", plot = plt, device = "png", path = wd$figs, scale = 1, width = 7, height = 4.5,   units = "in",  dpi = 1800)

######################## PopCurvCompare_spdaTraj.png ####################
...
plt = plot_grid(plt, plt2, align = "v", nrow = 2, rel_heights = c(0.42, 0.58))+theme(plot.background = element_rect(fill = "white", colour = NA))

#save figure
ggsave("PopCurvCompare_spdaTraj.png", plot = plt, device = "png", path = wd$figs, scale = 1, width = 7.5, height = 6.5,   units = "in",  dpi = 1600)

rm(plt,plt2,ex1,ex1.site,ex1.site_NoOvlp)
```
...
```{r, 'Comparing the two Popularity Curve Variants', echo=FALSE, out.width = '80%'}
knitr::include_graphics(paste0(wd$figs,"PopCurvCompare_spdaTraj.png"), FALSE)
```
...
Agg <- Agg %>% select(-PeriodLength,-PeriodBegin,-PeriodEnd, -OccuIntertia)
Agg = Agg[,-c(43:55)]
...
```{r, '02_FigureScript.R', echo=FALSE, message=FALSE, warning=FALSE}
invisible(source(paste0(wd$analysis,"02_FigureScript.R")))
```

```{r, 'Demographic rates', message=FALSE, warning=FALSE}

## Demographic rates rescaled (RS), Logged, and Absolute Value; their quantiles
Agg2 <- Agg2 %>% mutate(Abs_rPert = abs(rPert),
                        RS_rPert = scales::rescale(rPert),
                        LogRS_rPert = log(RS_rPert),
                        LogAbs_rPert = log(Abs_rPert),
                        Q.rPert = ecdf(rPert)(rPert),
                        Q.Abs_rPert = ecdf(Abs_rPert)(Abs_rPert),
                        Q.RS_rPert = ecdf(RS_rPert)(RS_rPert),
                        Q.LogRS_rPert = ecdf(LogRS_rPert)(LogRS_rPert),
                        Q.LogAbs_rPert = ecdf(LogAbs_rPert)(LogAbs_rPert),
                        Urb_Abs_rPert = abs(Urb_rPert),
                        Urb_RS_rPert = scales::rescale(Urb_rPert),
                        Urb_LogRS_rPert = log(Urb_RS_rPert),
                        Urb_LogAbs_rPert = log(Urb_Abs_rPert),
                        Q.Urb_rPert = ecdf(Urb_rPert)(Urb_rPert),
                        Q.Urb_Abs_rPert = ecdf(Urb_Abs_rPert)(Urb_Abs_rPert),
                        Q.Urb_RS_rPert = ecdf(Urb_RS_rPert)(Urb_RS_rPert),
                        Q.Urb_LogRS_rPert = ecdf(Urb_LogRS_rPert)(Urb_LogRS_rPert),
                        Q.Urb_LogAbs_rPert = ecdf(Urb_LogAbs_rPert)(Urb_LogAbs_rPert)) %>%
                  group_by(Period) %>% mutate(
                        Qp.rPert = ecdf(rPert)(rPert),
                        Qp.Abs_rPert = ecdf(Abs_rPert)(Abs_rPert),
                        Qp.RS_rPert = ecdf(RS_rPert)(RS_rPert),
                        Qp.LogRS_rPert = ecdf(LogRS_rPert)(LogRS_rPert),
                        Qp.LogAbs_rPert = ecdf(LogAbs_rPert)(LogAbs_rPert)) %>% ungroup()

  Agg2 <- do.call(data.frame, lapply(Agg2, function(x) replace(x, is.infinite(x), NA)))
